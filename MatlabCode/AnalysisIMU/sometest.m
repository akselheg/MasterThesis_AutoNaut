...
for dataRun = 1:1
    ...
    SingleCurrDir = str2num(SingleCurrentCell.dir(1:end,1:6));
    SingleCurrVel = str2num(SingleCurrentCell.vel(1:end,1:6));
    SingleCurrDir = interp1(SingleCurrentCell.timestamp(2:end), SingleCurrDir,GpsFix.timestamp);
    SingleCurrVel = interp1(SingleCurrentCell.timestamp(2:end), SingleCurrVel,GpsFix.timestamp);
    if Heave.src_ent==45
    ...
    for i= pad: n : length(GpsFix.sog) - pad
        if ~isnan(mean(SingleCurrVel(i:i+n)))
            ...
            VcDir = mean(ssa(rad2deg(SingleCurrDir(i:i+n)),'deg'));
            ...
            if mod(testingdata,5) 
                ...
                CurrentSpeed_data(idx + idx_offset) = mean(SingleCurrVel(i:i+n));
                ...
                test_CurrentSpeed_data(test_idx + test_idx_offset) = mean(SingleCurrVel(i:i+n));
